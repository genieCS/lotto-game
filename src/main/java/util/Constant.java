package util;

public class Constant {
    public static final int PURCHASE_UNIT = 1000;
    public static final int MINIMUM_LOTTO_NUM = 1;
    public static final int MAXIMUM_LOTTO_NUM = 45;
    public static final int LOTTO_SIZE = 6;
    public static final String PURCHASE_AMOUNT_MSG = "구입금액을 입력해 주세요.";
    public static final String WINNING_LOTTERY_MSG = "지난 주 당첨 번호를 입력해주세요";
    public static final String BONUS_BALL_MSG = "보너스 볼을 입력해주세요.";
    public static final String PURCHASE_COUNT_FORMAT = "%d개를 구매했습니다.";
    public static final int FIVE_WITH_BONUS = 7;
    public static final int mapSize = LOTTO_SIZE + 2;
    public static final String WINNING_RESULT = "당첨 통계\n---------";
    public static final String INTERSECTION_COUNT_FORMAT = "%d개 일치 (%d원)- %d개";
    public static final String BONUS_COUNT_FORMAT = "5개 일치, 보너스 볼 일치 (%d원)- %d개";
    public static final String EARNING_RATE_FORMAT = "총 수익률은 %f입니다.";
}
